...
if (~exist(fullfile([resDir, '-eval'], 'eval_bdry.txt'), 'file'))
    ...
    n = length(ids);
    ext = ids{1}(end -2:end);
    for i = 1:n, ids{i} = ids{i}(1:end - 4);
    end
    res = cell(1, n);
    for i = 1:n, res{i} = fullfile(resDir, [ids{i}, '.png']);
    end
    do = false(1, n);
    for i = 1:n, do(i) = ~exist(res{i}, 'file');
    end
    ids = ids(do);
    res = res(do);
    m = length(ids);
    parfor i = 1:m, id = ids{i};
        I = imread(fullfile(imgDir, [id, '.', ext]));
        D = [];
...
if (p.show), figure(p.show);
    edgesEvalPlot(resDir, name);
end
