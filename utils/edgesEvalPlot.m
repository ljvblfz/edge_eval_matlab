...
% parse inputs
if (nargin < 2 || isempty(nms)), nms = {};
end;
if (~iscell(nms)), nms = {nms};
end
if (nargin < 3 || isempty(cols)), cols = repmat({'r', 'g', 'b', 'k', 'm'}, 1, 100); end
if (~iscell(algs)), algs = {algs};
end;
if (~iscell(cols)), cols = {cols};
end
...
for f = 0.1:0.1:0.9, r = f:0.01:1;
    p = f .* r ./ (2 .* r - f); %f=2./(1./p+1./r)
    plot(r, p, 'Color', [0, 1, 0]);
    plot(p, r, 'Color', [0, 1, 0]);
end
...
[~, o] = sort(res(:, 4), 'descend');
res = res(o, :);
prs = prs(o);
cols = cols(o);
if (~isempty(nms)), nms = nms(o);
end
...
for i = n:-1:1
    ...
    if (~isempty(nms)), fprintf(' - %s', nms{i});
    end;
    fprintf('\n');
end
