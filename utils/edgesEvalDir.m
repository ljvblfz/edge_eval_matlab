...
if (exist(fNm, 'file'))
    ...
    R = R(:, 2);
    [~, o] = unique(P);
    R50 = interp1(P(o), R(o), max(P(o(1)), .5));
    varargout = [varargout, R50];
    return;
end

% perform evaluation on each image (this part can be very slow)
assert(exist(resDir, 'dir') == 7);
assert(exist(gtDir, 'dir') == 7);
ids = dir(fullfile(gtDir, '*.mat'));
ids = {ids.name};
n = length(ids);
do = false(1, n);
jobs = cell(1, n);
...
for i = 1:n
    ...
    [R, P, F] = computeRPF(cntR1, sumR1, cntP1, sumP1);
    [~, k] = max(F);
    ...
    oisCntR = oisCntR + cntR1(k);
    oisSumR = oisSumR + sumR1(k);
    oisCntP = oisCntP + cntP1(k);
    oisSumP = oisSumP + sumP1(k);
end
