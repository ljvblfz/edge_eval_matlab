  ...
  int nb, int n, float norm, int nOrients, bool full, bool interpolate )
{
    ...
    const __m128 _norm = SET(norm);
    const __m128 _oMult = SET(oMult);
    const __m128 _nbf = SET((float) nb);

    const __m128i _oMax = SET(oMax);
    const __m128i _nb = SET(nb);
  ...
  int bin, int nOrients, int softBin, bool full )
{
    ...
    for (x = 0; x < w0; x++) {
        ...
        gradQuantize(O + x * h, M + x * h,
                O0, O1, M0, M1, nb, h0, sInv2, nOrients, full, softBin >= 0);
